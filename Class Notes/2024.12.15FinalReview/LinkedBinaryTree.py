...
def count_paths_exceeding_threshold(root, threshold):
    ...
    #base case: root is leaf
        ...
        return 1 if root.data > threshold else 0
    #rec case
    paths = 0
    if root.left:
        paths += count_paths_exceeding_threshold(root.left, threshold-root.data)
    if root.right:
        paths += count_paths_exceeding_threshold(root.right, threshold-root.data)
    #ACTUALLY not needed bc problem says root-to-leaf paths only
    # if root.data > threshold: #count the internal nodes if threshold is already exceeded there
    #     paths += 1
    return paths

if __name__ == '__main__':
    node1 = LinkedBinaryTree.Node(8)
    node2 = LinkedBinaryTree.Node(1)
    node3 = LinkedBinaryTree.Node(2)
    node4 = LinkedBinaryTree.Node(3, node2, node3)
    node5 = LinkedBinaryTree.Node(5, node1, node4)
    t = LinkedBinaryTree(node5)

    print(count_paths_exceeding_threshold(node5, 9))
